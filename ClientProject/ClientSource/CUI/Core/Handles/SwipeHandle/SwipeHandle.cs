using System;
using System.Collections.Generic;
using System.Linq;
using System.Reflection;
using System.Diagnostics;
using Barotrauma;
using Microsoft.Xna.Framework;
using ComponentGenerator;

namespace CrabUI
{
  public class SwipeHandle : IModule, IAware
  {
    public object HostComponent { get; set; }
    public string HostPropName { get; set; }

    private ISwipeable host;
    [In]
    public ISwipeable Host
    {
      get => host;
      set
      {
        if (host is not null) DisconnectFromHost(host);
        host = value;
        if (host is not null) ConnectToHost(host);
      }
    }


    private void ConnectToHost(ISwipeable host)
    {
      host.MouseDown += Grab;
    }

    private void DisconnectFromHost(ISwipeable host)
    {
      host.MouseDown -= Grab;
    }

    public bool Active { get; set; }
    public bool Grabbed { get; private set; }
    public Vector2 PrevMousePos { get; private set; }


    private void Grab(CUIMouseEvent e)
    {
      if (!Active) return;
      if (!Host.TryGrab(this)) return;

      Grabbed = true;
      PrevMousePos = e.Pos;
      host.HubMouseMoved += Update;
      host.HubMouseUp += Release;
    }

    private void Release(CUIMouseEvent e)
    {
      Grabbed = false;
      host.HubMouseMoved -= Update;
      host.HubMouseUp -= Release;
      host.Release(this);
    }

    public void Update(CUIMouseEvent e)
    {
      Host.MoveChildrenOffset(e.Pos - PrevMousePos);
      PrevMousePos = e.Pos;
    }


  }
}