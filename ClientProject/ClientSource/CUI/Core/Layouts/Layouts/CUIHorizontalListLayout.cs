...
namespace CrabUI
{
  public class CUIHorizontalListLayout : Layout
  {
    ...
    public override void UpdateChildren()
    {
      ...
      {
        float w = 0;
        float h = Parent.InnerRect.Height; // Resize to host Height by default
      ...
      float emptySpace = Parent.InnerRect.Width - TotalWidth;
      ...
      if (Parent.Direction == CUIDirection.Straight)
      {
        ...
        foreach (ChildSize c in sizes)
        {
          c.Child.OuterRect = new CUIRect(
            Parent.InnerRect.Left + x + Parent.ChildrenOffset.X,
            Parent.InnerRect.Top + 0 + Parent.ChildrenOffset.Y,
            c.Width,
            c.Height
          );
      ...
      {
        float x = Parent.InnerRect.Width;
        foreach (ChildSize c in sizes)
        {
          x -= c.Width;
